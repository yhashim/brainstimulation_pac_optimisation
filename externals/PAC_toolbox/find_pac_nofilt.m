...
%  measure - measure to be used - it should be: 'esc', 'mi', 'cfc', 'mvl'
%  (added by YH)
%
...
pacmat = zeros(ybins, xbins);    
    ...
    for i = 1:ybins
        % for j = 1:xbins
        for j = 1:i % YH: edit to iterate y>x
            % Calculate matrix of PAC values
            if strcmp(measure, 'esc')
                pacmat(i,j) = esc_measure(sig_mod{1,j}, sig_pac{1,i}, 'y');
            end
            ...
            if strcmp(measure, 'mvl')
                pacmat(i,j) = mvl_measure(sig_mod{1,j}, sig_pac{1,i});
            end
