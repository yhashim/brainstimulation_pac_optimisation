...
for i2=1:xbins
    ...
    if strcmp(measure, 'esc')
        ...
        for i3=1:max(num_trials)
            ...
            % ph_filt_signals{1,i2}(:,i3) = phasevec((lower_bin + floor((upper_bin- lower_bin)/2)),sig2(:,i3), Fs, phfreq_bw);
        end
    end
...
for i2=1:ybins
    ...
    for i3=1:max(num_trials)
        ...
        % amp_filt_signals{1,i2}(:,i3) = ampvec((lower_bin + floor((upper_bin- lower_bin)/2)),sig1(:,i3), Fs, ampfreq_bw); 
