...
function read_Intan_RHD2000_file_no_dialog(path, file, workspace)
...
% Modified
% RT 17 May 2021
%  - Removed user dialog for automation
% RT 26 Jun 2022
%  - added 'workpsace' optional input argument to specify where to pass the
%    variables generated from the info file

% Default workpsace to use if not specified
if ~exist('workspace', 'var')
    workspace = 'base';
else
  if ~(strcmp(workspace, 'base') || strcmp(workspace, 'caller'))
    workspace = 'base';
  end
end
...
% new for version 2.01: moves filename info to specified workspace
% modified by RT 26 Jun 2022
filename = file;
assignin(workspace, 'filename', filename);
assignin(workspace, 'path', path);

assignin(workspace, 'notes', notes);
assignin(workspace, 'frequency_parameters', frequency_parameters);
if (data_file_main_version_number > 1)
    assignin(workspace, 'reference_channel', reference_channel);
end

if (num_amplifier_channels > 0)
    assignin(workspace, 'amplifier_channels', amplifier_channels);
    if (data_present)
        assignin(workspace, 'amplifier_data', amplifier_data);
        assignin(workspace, 't_amplifier', t_amplifier);
    end
    assignin(workspace, 'spike_triggers', spike_triggers);
end
if (num_aux_input_channels > 0)
    assignin(workspace, 'aux_input_channels', aux_input_channels);
    if (data_present)
        assignin(workspace, 'aux_input_data', aux_input_data);
        assignin(workspace, 't_aux_input', t_aux_input);
    end
end
if (num_supply_voltage_channels > 0)
    assignin(workspace, 'supply_voltage_channels', supply_voltage_channels);
    if (data_present)
        assignin(workspace, 'supply_voltage_data', supply_voltage_data);
        assignin(workspace, 't_supply_voltage', t_supply_voltage);
    end
end
if (num_board_adc_channels > 0)
    assignin(workspace, 'board_adc_channels', board_adc_channels);
    if (data_present)
        assignin(workspace, 'board_adc_data', board_adc_data);
        assignin(workspace, 't_board_adc', t_board_adc);
    end
end
if (num_board_dig_in_channels > 0)
    assignin(workspace, 'board_dig_in_channels', board_dig_in_channels);
    if (data_present)
        assignin(workspace, 'board_dig_in_data', board_dig_in_data);
        assignin(workspace, 't_dig', t_dig);
    end
end
if (num_board_dig_out_channels > 0)
    assignin(workspace, 'board_dig_out_channels', board_dig_out_channels);
    if (data_present)
        assignin(workspace, 'board_dig_out_data', board_dig_out_data);
        assignin(workspace, 't_dig', t_dig);
    end
end
if (num_temp_sensor_channels > 0)
    if (data_present)
        assignin(workspace, 'temp_sensor_data', temp_sensor_data);
        assignin(workspace, 't_temp_sensor', t_temp_sensor);
    end
end
